...
### WP_USER の空白を正規化している理由

WordPress の `sanitize_user()` は連続する空白を1つに詰めて保存する
(`preg_replace( '|\s+|', ' ', $username )`)。そのため保存済みのユーザー名に
空白が2つ並ぶことはない。

一方 `~/.marketik-wp` 側に空白が2つ入っていても**見た目では判別できず、
原因不明の401になるだけ**だった。WordPress と同じ正規化を
`load_config()` でも行い、この失敗を構造的に潰した。

（2026-09-21 に `takahashi keigo` という空白入りユーザー名を実際に
作成してしまった。ユーザー名は作成後に変更できないため、
スクリプト側で吸収できるようにしてある。）
